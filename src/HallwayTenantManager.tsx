...
import { cn } from "@/lib/utils"; // optional: or replace with a tiny helper below
...
// Jos sinulla ei ole cn-apuria, voit käyttää tätä:
// const cn = (...c: (string | false | null | undefined)[]) => c.filter(Boolean).join(" ");

/**
 * Käytävän asukasnäyttö – hallinta
 * - Vasen: editori kerroksille → asunnoille → asukkaille (sukunimet)
 * - Oikea: TV-esikatselu siitä, miten näyttö näyttää sisällön
 * - Tallennus: POST /api/hallways/:id (tai /api/ruutu/<serial>)
 *
 * Oletukset
 * - "Käytävä" kuuluu rakennukseen/kohteeseen. Se sisältää kerroksia, joilla on asuntoja ja 1–2 asukkaan sukunimiä per asunto.
 * - TV-sovellus noutaa GET /ruutu/<sarjanumero>.html ja renderöi staattisen sivun.
 */
...
export type Apartment = {
  id: string;
  number: string; // esim. "12A"
  tenants: Tenant[]; // suositus 1–2 sukunimeä
};
export type Floor = {
  id: string;
...
// ---------- API (korvaa omilla toteutuksilla) ----------
async function fetchHallway(hallwayId: string): Promise<Hallway> {
  const res = await fetch(`/api/hallways/${hallwayId}`);
  if (!res.ok) throw new Error("Käytävän lataus epäonnistui");
  return res.json();
}

type SaveResult = { ok: boolean; status?: number; statusText?: string; error?: string };
async function saveHallway(hallway: Hallway): Promise<SaveResult> {
  try {
    const res = await fetch(`/api/hallways/${hallway.id}`, {
      method: "POST",
...
function computePortraitCounts(n: number): number[] {
  ...
  if (map[n]) return map[n];
  // Fallback > 15: täytä 5/kolumni ja lisää uusi kolumni tarvittaessa
  const base = 5;
...
function buildColumnsShared(items: Floor[], orientation: Orientation): Floor[][] {
  ...
  for (let c = 0; c < counts.length; c++) {
    const take = counts[c];
    out.push(items.slice(idx, idx + take).reverse()); // näytä alhaalta→ylös kussakin kolumnissa
    idx += take;
  }
  return out;
}

// Julkaisu-kansio palvelimen juuressa
const RUUTU_DIR = "ruutu";

// Tallennusyritys palvelimelle ruutu/-kansioon. Backendin odotetaan kirjoittavan tiedoston
// polkuun /ruutu/<filename> ja palvelevan sen suoraan staattisena.
...
// --- LG TV: static HTML generation helpers ---
...
function buildStaticTvHtml(h: Hallway): string {
  ...
  // Rakennetaan sisältö ilman takaviivoja template-litteraaleissa
  const columnsHtml = cols.map((col) => {
    const floorsHtml = col.map((floor) => {
      const apartmentsHtml = floor.apartments.map((apt) => {
...
function openStaticPreviewTab(h: Hallway) {
  try {
    ...
    // Vapauta URL hetken päästä (uusi välilehti on jo ladannut sisällön)
...
export default function HallwayTenantManager({ hallwayId = "demo-hallway" }: { hallwayId?: string }) {
      ...
      try {
        ...
        if (!data.floors.length) {
          const f4: Floor = {
            ...
            level: 4,
            apartments: [
              { id: uid(), number: "401", tenants: [ { id: uid(), surname: "Koskinen" }, { id: uid(), surname: "Salmi" } ] },
              { id: uid(), number: "402", tenants: [ { id: uid(), surname: "Rautiainen" } ] },
              { id: uid(), number: "403", tenants: [ { id: uid(), surname: "Lehtonen" } ] },
              { id: uid(), number: "404", tenants: [ { id: uid(), surname: "Sukka" } ] },
            ],
          };
          const f3: Floor = {
            id: uid(),
            ...
            level: 3,
            apartments: [
              { id: uid(), number: "301", tenants: [ { id: uid(), surname: "Timonen" } ] },
              { id: uid(), number: "302", tenants: [ { id: uid(), surname: "Lahti" } ] },
              { id: uid(), number: "303", tenants: [ { id: uid(), surname: "Salmi" } ] },
              { id: uid(), number: "304", tenants: [ { id: uid(), surname: "Sepponen" } ] },
            ],
          };
          const f2: Floor = {
            id: uid(),
            ...
            level: 2,
            apartments: [
              { id: uid(), number: "201", tenants: [ { id: uid(), surname: "Siikanen" } ] },
              { id: uid(), number: "202", tenants: [ { id: uid(), surname: "Virtanen" }, { id: uid(), surname: "Niemi" } ] },
              { id: uid(), number: "203", tenants: [ { id: uid(), surname: "Sakki" } ] },
              { id: uid(), number: "204", tenants: [ { id: uid(), surname: "Pekkanen" } ] },
            ],
          };
          const f1: Floor = {
            id: uid(),
            ...
            level: 1,
            apartments: [
              { id: uid(), number: "101", tenants: [ { id: uid(), surname: "Kallio" }, { id: uid(), surname: "Mäkelä" } ] },
              { id: uid(), number: "102", tenants: [ { id: uid(), surname: "Tuuponen" } ] },
              { id: uid(), number: "103", tenants: [ { id: uid(), surname: "Salo" } ] },
              { id: uid(), number: "104", tenants: [ { id: uid(), surname: "Koskinen" } ] },
            ],
          };
          setHallway({ ...data, floors: [f4, f3, f2, f1], orientation: data.orientation || 'landscape' });
        } else {
  ...
  const addFloor = () => {
    setHallway(h => {
      ...
      return {
        ...
        floors: [
          ...h.floors,
          { id: uid(), label: `Kerros ${newLevel}`, level: newLevel, apartments: [] },
        ],
      };
    });
  };
  const deleteFloor = (floorId: string) => setHallway(h => ({ ...h, floors: h.floors.filter(f => f.id !== floorId) }));
  const updateFloor = (floorId: string, patch: Partial<Floor>) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? { ...f, ...patch } : f),
  }));

  const addApartment = (floorId: string) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
      apartments: [...f.apartments, { id: uid(), number: "", tenants: [ { id: uid(), surname: "" } ] }],
    } : f),
  }));

  const deleteApartment = (floorId: string, aptId: string) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
  ...
  const updateApartment = (floorId: string, aptId: string, patch: Partial<Apartment>) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
      apartments: f.apartments.map(a => a.id === aptId ? { ...a, ...patch } : a),
    } : f),
  }));

  const addTenant = (floorId: string, aptId: string) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
      apartments: f.apartments.map(a => a.id === aptId ? {
        ...a,
        tenants: [...a.tenants, { id: uid(), surname: "" }],
      } : a),
    } : f),
  }));

  const deleteTenant = (floorId: string, aptId: string, tenantId: string) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
      apartments: f.apartments.map(a => a.id === aptId ? {
        ...a,
        tenants: a.tenants.filter(t => t.id !== tenantId),
      } : a),
    } : f),
  }));

  const updateTenant = (floorId: string, aptId: string, tenantId: string, patch: Partial<Tenant>) => setHallway(h => ({
    ...h,
    floors: h.floors.map(f => f.id === floorId ? {
      ...f,
      apartments: f.apartments.map(a => a.id === aptId ? {
        ...a,
        tenants: a.tenants.map(t => t.id === tenantId ? { ...t, ...patch } : t),
      } : a),
    } : f),
  }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Luo staattinen LG HTML nykyisestä datasta
      const html = buildStaticTvHtml(hallway);
      const fname = staticFilenameFor(hallway);
      const relPath = `/${RUUTU_DIR}/${fname}`;

      // Yritä tallentaa palvelimelle (best-effort)
      const saveRes = await saveRuutu(hallway, html, fname);
      if (!saveRes.ok) {
        setServerSaveWarning(`Palvelintallennus epäonnistui (${saveRes.status ?? ''} ${saveRes.statusText ?? saveRes.error ?? ''}). Loin ja latasin HTML:n paikallisesti – muista siirtää tiedosto palvelimelle polkuun ${relPath} jotta TV löytää sen.`);
      } else {
        setServerSaveWarning("");
      }

      // Lataa HTML tiedostona paikallisesti
      downloadStaticHtmlFile(fname, html);

      // Näytä käyttäjälle absoluuttinen URL (olettaen, että tiedosto sijoitetaan originin juureen)
  ...
  // Dev-testit: sarakejako vastaa sääntötaulukkoja + HTML-metat ovat mukana
  useEffect(() => {
    const eq = (a: number[], b: number[]) => a.join(',') === b.join(',');
    const expectedLandscape: Record<number, number[]> = {
      1: [1], 2: [1,1], 3: [2,1], 4: [2,2], 5: [2,2,1], 6: [2,2,2],
      7: [2,2,2,1], 8: [2,2,2,2], 9: [3,2,2,2], 10: [3,3,2,2], 11: [3,3,3,2], 12: [3,3,3,3],
    };
    ...
    console.assert(new URL(`/${RUUTU_DIR}/ABC123.html`, window.location.origin).toString().includes(`/ruutu/ABC123.html`), 'Ruutu-URL muodostus ei toimi');
  }, []);

  // Test: esikatselun näkyvyys kytkimen mukaan
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const el = document.querySelector('[data-preview-card]');
    if (showPreview) {
      console.assert(!!el, 'TV-esikatselun pitäisi näkyä kun kytkin on päällä');
    } else {
      console.assert(!el, 'TV-esikatselun pitäisi olla piilossa kun kytkin on pois päältä');
    }
  }, [showPreview]);

  if (loading) {
    return (
      <div className="p-6 text-sm opacity-70">Ladataan hallintaa...</div>
    );
  }

  return (
    <div className={cn("p-4 lg:p-8 bg-[#dedede]", showPreview ? "grid grid-cols-1 lg:grid-cols-2 gap-6" : "flex justify-center")}>
      {/* Editori */}
      <div className={cn(showPreview ? "" : "w-full max-w-4xl")}>
        <Card className="shadow-lg">
          ...
          <CardContent>
            {/* Sarjanumero */}
            <div className="mb-4">
              <Label htmlFor="device-serial">Laitteen sarjanumero <span className="text-red-600" aria-hidden="true">*</span></Label>
              <Input
                id="device-serial"
                value={hallway.serial || ""}
                onChange={(e) => setHallway(h => ({ ...h, serial: e.target.value }))}
            ...
            {status && (
              <div className="mb-3 text-sm text-green-600">{status}</div>
            )}
            {serverSaveWarning && (
                            ...
                            <div key={apt.id} className="rounded-xl p-3 pb-4 bg-[#cccccc]">
                              <div className="grid grid-cols-12 gap-2 items-center">
                                <div className="col-span-12">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <div className="flex-1"><Input value={apt.number} onChange={(e) => updateApartment(floor.id, apt.id, { number: e.target.value })} placeholder={apartmentPlaceholder(floor.level, aptIdx)} className="w-full"/>
                                    </div>
                                    <div>
      ...
      {/* TV-esikatselu */}
      {showPreview && (
        <Card data-preview-card className="shadow-xl">
...
function HallwayTvPreview({ hallway }: { hallway: Hallway }) {
  ...
  // --- Dynaaminen sarakemäärä ja skaala ---
  ...
  // Rakenna sarakkeet: data jaetaan järjestyksessä, jokainen sarake käännetään (alhaalta ylös)
  ...
  // DEV "testit": varmista sääntölistat
  useEffect(() => {
    const expectedLandscape: Record<number, number[]> = {
  ...
  // Laske esikatselun ulkomitat niin, että pysty = vaaka mitat käänteisesti
  useEffect(() => {
    const updateBox = () => {
  ...
  useEffect(() => {
    const fitAndCols = () => {
      ...
      // Sarakemäärä sääntöjen mukaan orientation-mäppäyksellä
      ...
      // Skaalausvihje: landscape 7–9 hieman, 10+ enemmän; portrait ei pakoteta
    ...
    return () => { ro.disconnect(); window.removeEventListener('resize', fitAndCols); };
  }, [orientation, numCols, hallway, floorsAsc.length]);

  // Päivitä sarakkeet kun data tai sarakemäärä muuttuu
  useEffect(() => {
    setColumns(buildColumns(floorsAsc));
  }, [floorsAsc, orientation]);

  // Pienet ajonaikaiset tarkistukset
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} style={{ width: boxSize.w || undefined, height: boxSize.h || undefined }} className="bg-black text-white rounded-2xl p-5 overflow-hidden relative">
      ...
      <div
        ref={gridRef}
        style={{ transform: `scale(${(gridScale*scaleHint).toFixed(3)})`, transformOrigin: 'top left', gridTemplateColumns: `repeat(${Math.max(1, numCols)}, minmax(0, 1fr))` }}
                ...
                <div className="flex flex-col gap-3">
                  {floor.apartments.map((apt) => (
                    <div key={apt.id} className="grid grid-cols-[60px_1fr] gap-x-6">
                      ...
                      <div className="text-sm font-semibold break-words whitespace-normal">{apt.tenants.filter(t => t.surname.trim())[0]?.surname?.toUpperCase() || <span className="opacity-40">(tyhjä)</span>}</div>
                      {apt.tenants.filter(t => t.surname.trim()).slice(1).map((t, idx) => (
      ...
      {/* Alapalkki */}
      <div ref={footerRef} className="absolute left-0 right-0 bottom-0 text-[10px] opacity-70 p-2 text-center">
        Esikatselu · TV-sovellus noutaa tämän käytävän osoitteesta GET /api/hallways/:id
      </div>
    </div>
  );
}
